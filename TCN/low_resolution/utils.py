...
class TimeSeriesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        return self.X[index:index + self.seq_len].T, self.X[index + self.seq_len + self.y_offset - 1].T
...
def get_traffic_data(train_test_ratio=0.8):
    url_data = '/home/semin_noadmin/workspace/TCN/low_resol.csv'
    df_raw = pd.read_csv(url_data, index_col=0).fillna(0)
